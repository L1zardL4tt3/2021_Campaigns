...
$Cert = $(Get-ChildItem Cert:\CurrentUser\My\D341591D52987EC553BC56683C9A2547DA0BF2E6)
...
Function Decrypt-File
{
    ...
    $newName = [io.path]::GetFileNameWithoutExtension($f.FullName)
    Rename-Item -Path $f.FullName -NewName $newName
}


$files = get-childitem -path $HomeDirectory -Recurse | where {$_.extension -eq ".BillB"}
foreach ($f in $files) {
Write-Host $f.FullName
Decrypt-File $f.FullName $Cert
#$newName = [io.path]::GetFileNameWithoutExtension($f.FullName)
#Rename-Item -Path $f.FullName -NewName $newName
}
